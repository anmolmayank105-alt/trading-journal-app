    ...
    <AppLayout>
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
          <div>
            <h1 className={`text-xl sm:text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Trades</h1>
          ...
          <div className="flex gap-2 sm:gap-3">
            <button 
              ...
              className="btn-secondary flex items-center gap-1.5 sm:gap-2 text-sm sm:text-base px-3 sm:px-6 disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button 
              ...
              className="btn-primary flex items-center gap-1.5 sm:gap-2 text-sm sm:text-base px-3 sm:px-6"
            >
              <Plus className="w-4 h-4" />
              <span className="hidden xs:inline">Add</span> Trade
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className={`w-full ${isDark ? 'bg-white/5 border-white/10 text-white' : 'bg-white border-slate-300 text-slate-900'} border rounded-xl pl-10 sm:pl-12 pr-4 py-2.5 sm:py-3 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-colors duration-150 text-sm sm:text-base`}
            />
          </div>
          <button
            onClick={() => setShowFilters(!showFilters)}
            className={`btn-secondary flex items-center gap-2 text-sm sm:text-base ${showFilters ? 'bg-indigo-600/20 border-indigo-500/30' : ''}`}
          >
        ...
        {/* Filter Panel */}
        {showFilters && (
          <div className="card grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
            <div>
            ...
            <>
              <div className="overflow-x-auto -mx-px">
                <table className="w-full min-w-[600px]">
                  <thead>
                    <tr className={`text-left text-[10px] sm:text-xs ${isDark ? 'text-slate-400 bg-white/5' : 'text-slate-500 bg-slate-100'}`}>
                      <th className="px-2 sm:px-4 py-2 sm:py-3 font-medium">Symbol</th>
                      <th className="px-2 sm:px-4 py-2 sm:py-3 font-medium">Type</th>
                      <th className="px-2 sm:px-4 py-2 sm:py-3 font-medium hidden sm:table-cell">Qty</th>
                      <th className="px-2 sm:px-4 py-2 sm:py-3 font-medium">Entry</th>
                      <th className="px-2 sm:px-4 py-2 sm:py-3 font-medium">Exit</th>
                      <th className="px-2 sm:px-4 py-2 sm:py-3 font-medium">P&L</th>
                      <th className="px-2 sm:px-4 py-2 sm:py-3 font-medium hidden md:table-cell">R:R</th>
                      <th className="px-2 sm:px-4 py-2 sm:py-3 font-medium hidden sm:table-cell">Date</th>
                      ...
                      <tr key={trade.id} className="table-row">
                        <td className="px-2 sm:px-4 py-2 sm:py-3">
                          <div className="flex items-center gap-1.5 sm:gap-3">
                            <div className={`p-1 sm:p-2 rounded-lg ${trade.tradeType === 'long' ? 'bg-emerald-500/20' : 'bg-red-500/20'}`}>
                            ...
                            <div>
                              <div className="font-medium text-white text-xs sm:text-sm truncate max-w-[80px] sm:max-w-none">{trade.symbol}</div>
                              <div className={`text-[10px] sm:text-xs ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                                {trade.exchange} • <span className="uppercase">{trade.segment}</span>
                              </div>
                            </div>
                          </div>
                        </td>
                        <td className="px-2 sm:px-4 py-2 sm:py-3">
                          <span className={`badge text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 ${trade.tradeType === 'long' ? 'badge-success' : 'badge-danger'}`}>
                            {trade.tradeType === 'long' ? 'BUY' : 'SELL'}
                          </span>
                        </td>
                        <td className="px-2 sm:px-4 py-2 sm:py-3 text-slate-300 text-xs sm:text-sm hidden sm:table-cell">{trade.quantity}</td>
                        <td className="px-2 sm:px-4 py-2 sm:py-3 text-slate-300 text-[10px] sm:text-sm">₹{trade.entryPrice.toLocaleString()}</td>
                        <td className="px-2 sm:px-4 py-2 sm:py-3 text-slate-300 text-[10px] sm:text-sm">
                        ...
                        <td className="px-2 sm:px-4 py-2 sm:py-3">
                          {trade.status === 'closed' && trade.exitPrice ? (
                            (() => {
                              ...
                              return (
                                <span className={`font-medium text-[10px] sm:text-sm ${pnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                  {pnl >= 0 ? '+' : ''}₹{pnl.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                                </span>
                              );
                            })()
                          ) : (
                            <span className="badge badge-warning text-[10px] sm:text-xs px-1 sm:px-2">OPEN</span>
                          )}
                        </td>
                        <td className="px-2 sm:px-4 py-2 sm:py-3 hidden md:table-cell">
                          {trade.riskRewardRatio !== undefined && trade.riskRewardRatio !== null ? (
                            <span className={`font-medium text-xs sm:text-sm ${trade.riskRewardRatio >= 1 ? 'text-emerald-400' : 'text-amber-400'}`}>
                        ...
                        <td className="px-2 sm:px-4 py-2 sm:py-3 text-slate-400 text-[10px] sm:text-sm hidden sm:table-cell">
                        ...
                        <td className="px-2 sm:px-4 py-2 sm:py-3">
                          <div className="flex items-center gap-0.5 sm:gap-1">
                            <Link
                              ...
                              className="p-1 sm:p-2 text-slate-400 hover:text-white hover:bg-white/10 rounded-lg transition-colors"
                              title="View"
                            >
                            ...
                            <button
                              ...
                              className="p-1 sm:p-2 text-slate-400 hover:text-indigo-400 hover:bg-indigo-400/10 rounded-lg transition-colors"
                              title="Edit"
                            >
                            ...
                            <button
                              ...
                              className="p-1 sm:p-2 text-slate-400 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-colors"
                              title="Delete"
                            >
              ...
              {/* Pagination */}
              {totalPages > 1 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-2 px-3 sm:px-4 py-3 sm:py-4 border-t border-white/5">
                  ...
                  <div className="flex items-center gap-1 sm:gap-2">
                    <button
                      ...
                      className="p-1.5 sm:p-2 text-slate-400 hover:text-white hover:bg-white/10 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5" />
                    </button>
                    <span className="text-[10px] sm:text-sm text-slate-300">

...
export default function MistakeAnalysis({ data, period }: MistakeAnalysisProps) {
  ...
  // Prepare chart data with shorter labels
  const chartData = data.map(item => ({
    name: item.mistake.length > 30 ? item.mistake.substring(0, 30) + '...' : item.mistake,
    count: item.count,
    fullName: item.mistake,
    percentage: (item.count / data.reduce((sum, m) => sum + m.count, 0)) * 100,
  }));
      ...
      <div className="mt-6 pt-4 border-t border-slate-700 grid grid-cols-3 gap-4">
        ...
        <div>
          ...
          <p className="text-sm font-semibold text-red-400">
            {data[0]?.mistake.substring(0, 15)}{data[0]?.mistake.length > 15 ? '...' : ''}
          </p>
        </div>
        <div>
          <p className="text-xs text-slate-400 mb-1">Total Impact</p>
          <p className="text-lg font-semibold text-red-400">
            ₹{Math.abs(data.reduce((sum, m) => sum + m.totalImpact, 0)).toLocaleString('en-IN', { maximumFractionDigits: 0 })}
          </p>
        </div>
      </div>
    </div>
